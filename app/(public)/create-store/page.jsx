...
export default function CreateStore() {
    ...
    const onSubmitHandler = async (e) => {
        ...
        if (!user) {
            return toast('please login to contione')
        }
        ...
        try {
            ...
            toast.error(error?.response?.data?.error || error.messagex)
        }
    ...
    if (!user) {
        return (
            <div className="min-h-[80vh] mx-6 flex items-center justify-center text-slate-400">
                <h1 className="text-2xl sm:text-4xl font-semibold">Please <span className="text-slate-500">Login</span> to contione</h1>
            </div>
        )
    }
